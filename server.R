...
shinyServer(function(input, output, session) {
  output$netPlot <- ndtv:::renderNdtvAnimationWidget({
    ...
    withProgress(message = 'Making plot:', value = 0, {
      ...
      roundsOfPlay <-
        100 # number of rounds of play with each generations
      perfectCommunication <- FALSE
      ...
      generation <-
        10 # rounds for which the state of the world is the same
      threshold <- 2
      ...
      for (round in 1:roundsOfPlay) {
        ...
        incProgress(1 / roundsOfPlay,
                    detail = paste("rendering network", round, sep = " "))
      ...
      vertexToolTip <- c()
      reliablity.epistemic.vector <-
        round(reliablity.epistemic.vector, digits = 2)
      reliablity.social.vector <-
        round(reliablity.social.vector, digits = 2)
      vertexToolTip[1] <- paste(
        '<strong>',
        'Nature',
        '</strong>',
        '<br>',
      ...
      render.d3movie(
        ...
        plot.par = list(
          bg = 'white',
          vertex.cex = 1.5,
          vertex.col = c("#d10d0d", rep("gray20", N)),
          vertex.border = c("#d10d0d", rep("gray20", N)),
          vertex.tooltip = vertexToolTip,
          edge.col = "darkgray",
          label.col = c("#d10d0d", rep("gray20", N)),
          label.cex = 1,
          displaylabels = TRUE
        ),
        ...
        d3.options = list(
          ...
          extraPlotCmds = NULL,
          initial.coords = 1000,
          plot.par = list(bg = 'white', displaylabels = FALSE)
        ),
        animation.mode = 'MDSJ',
